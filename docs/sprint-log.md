...
doing schoolwork and buy a bike or a jeep to ride where other kids can see
it, so the park ships EMPTY of them and owning one means something. Nothing
was deleted: prop_db entries, drive motion, probe_drive and the seeder all
stay, because they are inventory waiting for a shop.

**The mechanism.** `engine/assets/map_edits.json` (committed, generated by
`tools/engine/gen_map_edits.mjs`) lists 98 placement tags — 18 vehicles + 80
attached parts, labelled by catalogue item; park.js drops them at load,
BEFORE collision / prop DB / library are built, so a dropped placement
genuinely does not exist. Delete a row and that placement comes back.
`?edits=0` boots the full map — probe_props, probe_drive and the seeder go
through that door, so all 32 mounts and 9 drives stay verified even though
the shipping park no longer places them. Park: 1103 → 1005 placements,
275 → 208 placed prototypes (67 became shop-only; their meshes stay loaded
but disabled — a mesh with no thin-instance buffer would draw itself at the
origin).

**The catalogue.** `engine/assets/catalogue.json`: the 14 buyable items with
prototype, display-name placeholder, and the attached-part ASSEMBLY (each
part's transform relative to its vehicle) measured off the real map
placements before removal — the data a shop spawner needs and the map would
otherwise have carried. Found while building it: the map assembles Bike_02
from Bike_01's pedal/peg/badge prototypes and Scooter_02 from Scooter_01
parts, so prop_db's `parts` arrays under-count what sits on a vehicle (65 vs
the name-prefix rule's 80). The catalogue records map truth.

**Probes that described the past, updated:** probe_boot (1103/275 →
1005/208 + a "jeep is really gone" check), probe_objects (bulk-removal
pattern was all vehicles — now prams/toy trucks/balls), probe_library
(cards checked against placed-prototype truth, not 275). The tour's jeep +
skateboard scenes became the seesaw (lone rider sinks, plank remembers) and
the roundabout. All nine probes green before and after.
