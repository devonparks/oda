...
/**
 * COMMITTED MAP EDITS — placements the engine drops at load.
 *
 * Devon's call (2026-08-01): every rideable thing is shop inventory. Kids
 * will buy a bike or a jeep with coins earned from schoolwork and ride it
 * where other kids can see it, so the park ships EMPTY of them. The edits
 * file lives in engine/assets (NOT world/assets — the three.js park at
 * /world stays untouched) and lists one tag per dropped placement, labelled
 * with the catalogue item it belongs to; delete a row and that placement
 * comes back. Generated by tools/engine/gen_map_edits.mjs.
 *
 * Applied HERE, before anything else sees the rows, so a dropped placement
 * genuinely does not exist: no thin instance, no collider (collision.js
 * reads park.items), no mountable spot (props.js reads park.items), no
 * library card (library.js counts park.items). This is a different thing
 * from the object layer's localStorage removals, which only zero matrices
 * of instances that were built.
 *
 * `?edits=0` boots the full map — the probes ride the shop inventory
 * through that door so every catalogue item's mount data stays verified.
 */
async function loadMapEdits() {
  if (/[?&]edits=0/.test(location.search)) return new Set();
  const v = window.__AMG_BUILD ? ('?v=' + window.__AMG_BUILD) : '';
  try {
    const edits = await (await fetch(new URL('../assets/map_edits.json', import.meta.url).href + v)).json();
    return new Set((edits.removed || []).map((r) => r.tag));
  } catch (e) {
    console.warn('[park] map_edits.json unavailable — booting the full map:', e.message);
    return new Set();
  }
}
...
export async function loadPark(scene, onProgress = () => {}) {
  ...
  const [layout, dropTags] = await Promise.all([
    (await fetch(ASSET_BASE + 'park_layout.json')).json(),
    loadMapEdits(),
  ]);
  ...
  for (let i = 0; i < layout.items.length; i++) {
    ...
    // committed map edits: the row never becomes an item, so collision, the
    // prop database and the library all agree it does not exist
    if (dropTags.size && dropTags.has(row.n + '@' + row.p.map((n) => n.toFixed(2)).join(','))) {
      dropped++;
      continue;
    }
  ...
  // A prototype whose every placement was dropped must not draw: a mesh with
  // no thin-instance buffer renders ITSELF at the origin. It stays loaded —
  // the shop will spawn these — just disabled.
  for (const [name, m] of byName) {
    if (!groups.has(name)) m.setEnabled(false);
  }
  ...
  const stats = {
    ...
    removed: dropped,
    skipped: layout.items.length - instances - dropped,
  ...
  console.log(`[park] ${stats.prototypes} prototypes, ${stats.placements} placements, `
    ...
    + (dropped ? `, ${dropped} placements dropped by map edits` : ''));
